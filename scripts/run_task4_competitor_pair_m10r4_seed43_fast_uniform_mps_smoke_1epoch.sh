...
# M10-R4 competitor-calibrated carrier, first batch-negative competitor screen,
...
EXPERIMENT_ID="task4_competitor_pair_m10r4_smoke"
EXPERIMENT_DESIGN_NOTE="2026-07-09 020034 CCFCRec Amazon-VG M10-R4 competitor-calibrated carrier 方法设计.md"
EXPERIMENT_ROUTE_NOTE="2026-07-09 020204 CCFCRec Amazon-VG M10-R4 competitor-calibrated carrier 路线判断.md"
EXPERIMENT_IMPLEMENTATION_PLAN="2026-07-09 020956 CCFCRec Amazon-VG M10-R4 competitor pair carrier 实现计划.md"
...
printf '%s\n' "${RESULT_ROOT}" > "${REPO_ROOT}/task4_competitor_pair_m10r4_smoke_latest_result_root.txt"

{
  printf 'EXPERIMENT_ID=%s\n' "${EXPERIMENT_ID}"
  printf 'EXPERIMENT_DESIGN_NOTE=%s\n' "${EXPERIMENT_DESIGN_NOTE}"
  printf 'EXPERIMENT_ROUTE_NOTE=%s\n' "${EXPERIMENT_ROUTE_NOTE}"
  printf 'EXPERIMENT_IMPLEMENTATION_PLAN=%s\n' "${EXPERIMENT_IMPLEMENTATION_PLAN}"
  ...
  printf 'METHOD_VARIANTS=task4_competitor_pair task4_competitor_pair_shuffle task4_competitor_pair_rsp_control task4_competitor_pair_acat_control\n'
...
for alpha in ${TASK4_COMPETITOR_ALPHA_LIST}; do
  ...
  run_one "R4a${label}_real" task4_competitor_pair "${alpha}"
  run_one "R4a${label}_shuffle" task4_competitor_pair_shuffle "${alpha}" --task4_shuffle_seed "${SEED}"
  run_one "R4a${label}_rsp" task4_competitor_pair_rsp_control "${alpha}"
  run_one "R4a${label}_acat" task4_competitor_pair_acat_control "${alpha}"
done
