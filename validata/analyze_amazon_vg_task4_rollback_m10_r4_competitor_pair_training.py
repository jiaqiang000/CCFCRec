#!/usr/bin/env python3
"""
CCFCRec Amazon-VG M10-R4 competitor-pair training analysis.

Offline analysis for finished M10-R4 runs. It parses validation curves from the
real/shuffle/RSP-control/Acat-control branches and applies the pre-registered
seed43 gate before deciding whether multi-seed training should open.
"""
...
DEFAULT_RESULT_ROOT = Path(
    ...
    "2026-07-09_022004_task4_competitor_pair_m10r4_seed43_workers8_fast_uniform_mps_100epoch"
)
ANALYSIS_SCRIPT = "validata/analyze_amazon_vg_task4_rollback_m10_r4_competitor_pair_training.py"
TOTAL_DESIGN_NOTE_NAME = "2026-07-09 010147 CCFCRec Amazon-VG Task4-rollback M10-R recoverability and carrier audit 总设计"
R4_DESIGN_NOTE_NAME = "2026-07-09 020034 CCFCRec Amazon-VG M10-R4 competitor-calibrated carrier 方法设计"
R4_ROUTE_NOTE_NAME = "2026-07-09 020204 CCFCRec Amazon-VG M10-R4 competitor-calibrated carrier 路线判断"
R4_IMPLEMENTATION_PLAN_NAME = "2026-07-09 020956 CCFCRec Amazon-VG M10-R4 competitor pair carrier 实现计划"
M9_ROUTE_NOTE_NAME = "2026-07-08 225222 CCFCRec Amazon-VG Task4-revise-4 M9 q-only alpha sweep 路线判断"

M9_REAL_SHUFFLE_GATE_ABS = 0.000647
TARGET_NET_GATE_ABS = 0.0015
THREE_PCT_ABS_GATE = 0.003611
EARLY_TRANSIENT_EPOCH_RATIO = 0.20

ROLE_BY_METHOD_VARIANT = {
    "task4_competitor_pair": "real",
    "task4_competitor_pair_shuffle": "shuffle",
    "task4_competitor_pair_rsp_control": "rsp_control",
    "task4_competitor_pair_acat_control": "acat_control",
}
...
def _role_from_variant(method_variant: str) -> str:
    ...
    if role is None:
        raise ValueError(f"unknown R4 method_variant: {method_variant}")
    return role
...
def _run_label(role: str, alpha: Any) -> str:
    return f"R4{_alpha_tag(alpha)}_{role}"
...
def build_validation_curve(result_root: Path | str) -> pd.DataFrame:
    ...
    for run_dir in _run_dirs(root):
        ...
        rows.append(result)
    if not rows:
        raise ValueError(f"no R4 result.csv/run_config.json pairs found under: {root}")
...
def decide_route(summary: pd.DataFrame, pair_comparison: pd.DataFrame) -> dict[str, Any]:
    ...
    if not completed:
        ...
        open_multi_seed = False
        open_alpha_sweep = False
    elif passes_m9 and passes_target and not_losing_rsp and not_losing_acat and not_early_transient:
        route = "r4_competitor_pair_ready_for_multi_seed"
        next_action = "run_multi_seed_confirmation_same_protocol"
        reason = "real-shuffle clears M9 and target gates while real does not lose to controls"
        open_multi_seed = True
        open_alpha_sweep = False
    elif passes_m9 and not_losing_rsp and not_losing_acat and not_early_transient:
        route = "r4_competitor_pair_weak_positive_not_multiseed_ready"
        next_action = "do_not_open_multiseed_redesign_boundary_competitor_sampling"
        reason = "real-shuffle barely clears M9 gate but misses the +0.0015 target gate"
        open_multi_seed = False
        open_alpha_sweep = False
    elif not not_losing_rsp or not not_losing_acat:
        route = "r4_competitor_pair_control_dominated"
        next_action = "do_not_sweep_alpha_return_to_control_aware_boundary_sampling"
        reason = "real branch does not beat one or more RSP/Acat controls"
        open_multi_seed = False
        open_alpha_sweep = False
    else:
        route = "r4_competitor_pair_no_reliable_gain"
        next_action = "stop_current_competitor_pair_variant_redesign_sampling"
...
def write_result_markdown(
    ...
    content = f"""---
title: {run_stamp} CCFCRec Amazon-VG M10-R4 competitor pair training 结果
date: {run_stamp[:10]}
tags:
  - CCFCRec
  - Amazon-VG
...
# {run_stamp} CCFCRec Amazon-VG M10-R4 competitor pair training 结果

## Material Passport

- artifact_type: experiment_training_result
- project: CCFCRec Amazon-VG category availability
- stage: M10-R4 competitor pair carrier
- status: analyzed
- result_root: `{result_root}`

> [!info] 来源说明
> 上游总设计：[[{TOTAL_DESIGN_NOTE_NAME}]]
> R4 方法设计：[[{R4_DESIGN_NOTE_NAME}]]
> R4 路线判断：[[{R4_ROUTE_NOTE_NAME}]]
> R4 实现计划：[[{R4_IMPLEMENTATION_PLAN_NAME}]]
> M9 路线判断：[[{M9_ROUTE_NOTE_NAME}]]
...
解释：本轮评估 task4_competitor_pair（竞争用户成对载体）相对 shuffle（打乱负控）、RSP control（RSP 对照）、Acat control（类别可用性对照）的 validation（验证集评估）净收益。seed43（随机种子43）未达到 +0.0015 的后续 multi seed（多随机种子）准入目标时，不开放多种子复验，也不继续 alpha（强度）大扫。
...
> [!info] 字段说明
> `best_ndcg@20`：该分支 validation（验证集评估）曲线上的最佳 NDCG@20。
> `best_ndcg_peak_minus_last`：最佳点减最后一轮，用来观察 transient gain（短暂收益）。
> `early_transient_peak_flag`：最佳点是否过早出现。
...
> [!info] 字段说明
> `delta_best_ndcg@20`：real（真实载体）最佳 NDCG@20 减 control（对照）最佳 NDCG@20。
> `passes_m9_net_gate`：是否超过 M9（上一轮方法）real-shuffle（真实减打乱）净差 +0.000647。
> `passes_target_net_gate`：是否达到 R4 设计中的理想准入净差 +0.0015。
...
def build_outputs(output_root: Path, run_stamp: str) -> Outputs:
    output_dir = output_root / f"{run_stamp} task4-rollback-m10-r4-competitor-pair-training-analysis"
    return Outputs(
        output_dir=output_dir,
        validation_curve_csv=output_dir / "m10_r4_validation_curve.csv",
        validation_summary_csv=output_dir / "m10_r4_validation_summary.csv",
        pair_comparison_csv=output_dir / "m10_r4_pair_comparison.csv",
        route_decision_json=output_dir / "m10_r4_route_decision.json",
        manifest_json=output_dir / "run_manifest.json",
        result_md=output_dir / f"{run_stamp} CCFCRec Amazon-VG M10-R4 competitor pair training 结果.md",
    )
...
def run(args: argparse.Namespace) -> Outputs:
    ...
    manifest = {
        ...
        "run_date": run_date,
        "run_iso": run_iso,
        "experiment_stage": "M10-R4",
        "analysis_script": ANALYSIS_SCRIPT,
        "result_root": str(result_root),
        "design_note": R4_DESIGN_NOTE_NAME,
        "route_note": R4_ROUTE_NOTE_NAME,
        "implementation_plan": R4_IMPLEMENTATION_PLAN_NAME,
        "gates": {
            "m9_real_shuffle_gate_abs": M9_REAL_SHUFFLE_GATE_ABS,
            "target_net_gate_abs": TARGET_NET_GATE_ABS,
            "three_pct_abs_gate": THREE_PCT_ABS_GATE,
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Analyze M10-R4 competitor-pair training results.")
